library(readr)
library(dplyr)
probs <- read_csv("2LevelDesc_woTotal.csv")
head(probs)
names(probs)

#count nlevel for every Category/Attribute
#probs2 <-  probs %>% group_by(Category, Attribute) %>% dplyr::summarize(nlevel = 5)
...
names <- c("Category","Level","Attribute","Region")
probs[,names] <- lapply(probs[,names] , factor)

levels(probs$Attribute)
levels(probs$Category)
...
resultDI = result
write.csv(resultDI,"resultDI_2L.csv")
